 ...
 * @param <T>
 */
public class SequentialReaderImpl<A>
    implements SequentialReader<A>
{
    private static final Logger logger = LoggerFactory.getLogger(SequentialReaderImpl.class);

    protected PageHelperForConsumer<A> pageHelper;
    ...
    public SequentialReaderImpl(SmartRangeCacheImpl<T> cache, Range<Long> requestRange) { //SmartRangeCacheImpl<T> cache, Range<Long> requestRange) {
        super();
        this.cache = cache;
