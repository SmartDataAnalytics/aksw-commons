...
public class AggBuilderTests {
    ...
    public void aggInputBroadcastMapTest() {
        ...
        ParallelAggregator<String, Void, Integer, ?> subAgg = AggBuilder.inputTransform(String::length, AggBuilder.maxInteger());

        Map<String, ParallelAggregator<String, Void, Integer, ?>> aggMap = new HashMap<>();
        aggMap.put("strLen", subAgg);

        AggInputBroadcastMap<String, Void, String, Integer> agg = AggBuilder.inputBroadcastMap(aggMap);

        AccInputBroadcastMap<String, Void, String, Integer> acc1 = agg.createAccumulator();
        AccInputBroadcastMap<String, Void, String, Integer> acc2 = agg.createAccumulator();

        acc1.accumulate("test");
        acc2.accumulate("testtest");

        AccInputBroadcastMap<String, Void, String, Integer> acc = agg.combine(acc1, acc2);
    ...
    public void aggInputSplitWithFixedKeysTest() {
        ...
        ParallelAggregator<String, Void, Map<String, Long>, ?> agg = AggBuilder.inputSplit(
                new LinkedHashSet<>(Arrays.asList("a", "b", "c")), true,
                input -> Collections.singleton(input),
                (input, key) -> key, AggBuilder.counting());

        Accumulator<String, Void, Map<String, Long>> acc = agg.createAccumulator();
        acc.accumulate("b");
        acc.accumulate("c");
        acc.accumulate("c");
        acc.accumulate("d");
